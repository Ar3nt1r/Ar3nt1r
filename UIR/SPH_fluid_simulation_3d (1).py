...
#
CONSTANT_FORCE = np.array([[0.0, -0.1, -0.1]])
    ...
    for iter in tqdm(range(N_TIME_STEPS)):
        if iter % ADD_PARTICLES_EVERY == 0 and n_particles < MAX_PARTICLES:
            new_positions = np.array([
                [10 + np.random.rand(), DOMAIN_Y_LIM[1], 10 + np.random.rand()],
                [15 + np.random.rand(), DOMAIN_Y_LIM[1], 15 + np.random.rand()],
                [20 + np.random.rand(), DOMAIN_Y_LIM[1], 20 + np.random.rand()]
            ])
        ...
        #
        out_of_front_boundary = positions[:, 2] > DOMAIN_Z_LIM[1]
        out_of_back_boundary = positions[:, 2] < DOMAIN_Z_LIM[0]
        ...
        positions[out_of_top_boundary, 1] = DOMAIN_Y_LIM[0]

        velocities[out_of_back_boundary, 2] *= DAMPING_COEFFICIENT
        positions[out_of_back_boundary, 2] = DOMAIN_Z_LIM[0]

        velocities[out_of_front_boundary, 2] *= DAMPING_COEFFICIENT
        positions[out_of_front_boundary, 2] = DOMAIN_Z_LIM[1]
